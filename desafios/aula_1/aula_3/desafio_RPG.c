...
void removeDaListaCircular(ListaCircular lista, Celula alvo) {
      ...
      Celula anterior;
      anterior = lista->inicio; //Pra encontrar o nó que está antes do alvo

    if (listaVazia(lista) || alvo == NULL)
        return;

    // se o alvo é o único elemento 
    ...
    while (anterior->prox != alvo &&
           anterior->prox != lista->inicio) { //Loop até o prox de "anterior" ser o alvo. Quando for, o anterior de fato 
        anterior = anterior->prox;  //vai ser o anterior do alvo. 
    }

       if (anterior->prox != alvo) //verificacao para caso o while pare pela lista completar o circulo, e nao por achar o "anterior"
        return;


    anterior->prox = alvo->prox; //remove o alvo

    // se o alvo é o início
...
Celula executaUmTurno(ListaCircular lista, Celula atual) {
    ...
    double numAleatorio = numeroAleatorio();
...
/* =========================================================
   ...
   //Teste muito legal da Maria:

   void testeMary(void) {
       ListaCircular rpg = novaListaCircular();
       Celula atual;
        int turno = 1;


    Personagem a = {
        "Kaiser Cohen", JOGADOR, "Combatente", 50, 20, 3, 1, {"Sniper", 1.5f}
    };

    Personagem b = {
        "Joui Jouki", JOGADOR, "Combatente", 70, 15, 5, 1, {"Katana", 2.0f}
    };

    Personagem c = {
        "O Viajante", MONSTRO, "Criatura de Energia", 60, 20, 2, 0, {"", 0.0f}
    };

    Personagem d = {
        "O Espreitador", MONSTRO, "Coitadinho", 50, 15, 1, 0, {"", 0.0f}
    };

    insereOrdenadoPorIniciativa(rpg, a);
    insereOrdenadoPorIniciativa(rpg, b);
    insereOrdenadoPorIniciativa(rpg, c);
    insereOrdenadoPorIniciativa(rpg, d);

    srand(42);

    printf("=== COMBATE COMPLETO ===\n");
    printf("Estado inicial:\n");
    printListaCircular(rpg);

    atual = rpg->inicio;

    while (contarTipo(rpg, JOGADOR) > 0 &&
           contarTipo(rpg, MONSTRO) > 0) {

        printf("\n----- TURNO %d -----\n", turno);

        atual = executaUmTurno(rpg, atual);
    ...
    if (contarTipo(rpg, JOGADOR) > 0)
        printf("VITORIA DA ORDEM!\n");
    else
        printf("VITORIA DAS CRIATURAS!\n");

    liberarLista(rpg);

   }

   //Testes muito legais do Guilherme:
